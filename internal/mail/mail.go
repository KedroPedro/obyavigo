...
func (m *Mail) SendRegConfirm(email string) (string, error) {
	...
	msg.SetBodyString(mail.TypeTextPlain,
		"Перейдите по этой ссылке для подтверждения регистрации: https://localhost:8443/api/auth/confirm-email/"+cipheredMail)
...
func (m *Mail) SendPasswordReset(email, token string) error {
	...
	msg.SetBodyString(mail.TypeTextPlain,
		"Для восстановления пароля перейдите по ссылке: https://localhost8443/reset-password?token="+token+"\n")
